...
//Use cases
const subscriptionUseCase = new SubscriptionUseCase(cartRepository,paymentRepository,subscriptionRepository,couponRepository,userRepository,generateQrCode,generateEmail,gymRepository)
const bookTrainerUseCase = new BookTrainerUseCase(paymentRepository,bookTrainerRepository)

//controllers
const paymentController = new PaymentController(subscriptionUseCase,userRepository,bookTrainerUseCase);
...
router.post('/webhook/book_trainer',express.raw({type:'application/json'}), (req,res)=> paymentController.bookTrainerPayment(req,res))
