...
class BookTrainerUseCase {
  ...
  constructor(
    ...
    trainerRepository: BookTrainerRepository
  ) {
  ...
  async verifyTrainerBooking(body: any) {
    const isAlreadyTrainerBooked =
      await this._TrainerRepository.isAlreadyBooked(
        body.userId,
        body.trainerId
      );
    ...
    if (!isAlreadyTrainerBooked) {
      const sessionId =
        await this._PaymentRepository.confirmBookTrainerPayment(body);
  ...
  async bookTrainerPayment(bookTrainerData: any) {
    await this.confirmBooking(bookTrainerData);
  }
}
