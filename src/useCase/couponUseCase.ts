...
class CouponUseCase {
  ...
  async editCoupon(couponId: string, data: CouponI) {
    if (couponId) {
      const couponExist = await this._CouponRepository.findByNameForEdit(
        data.name,
        couponId
      );
      if (!couponExist) {
        const coupon: CouponI = await this._CouponRepository.findByIdAndUpdate(
          couponId,
          data
        );
  ...
  async deleteCoupon(id: string, data: string) {
  ...
  async couponValidateForCheckout(userId: string, name: string, price: number) {
    const coupon: CouponI =
      await this._CouponRepository.findByNameForCheckout(name);
    if (coupon) {
      if (
        new Date(coupon.startingDate) > new Date() ||
        new Date() > new Date(coupon.endingDate)
      ) {
        return {
          status: 400,
